...
if [[ -n "${SAM_REVIEW_CONTINUOUS_WORKSPACE:-}" ]]; then
  ...
  progress_args+=(--snapshot-size "${SAM_REVIEW_SNAPSHOT_SIZE:-2500}")
...
training_args=()
if [[ -n "${SAM_REVIEW_TRAINING_WORKSPACE:-}" ]]; then
  training_args+=(--training-workspace "${SAM_REVIEW_TRAINING_WORKSPACE}")
fi
