...
INFERENCE_CONCURRENCY=${SAM_CONTINUOUS_SOURCE_SCAN_INFERENCE_CONCURRENCY:-8}
TARGET_SCAN_WINDOWS=${SAM_CONTINUOUS_SOURCE_SCAN_TARGET_WINDOWS:-3600}
MAX_WINDOW_HOP_SECONDS=${SAM_CONTINUOUS_SOURCE_SCAN_MAX_WINDOW_HOP_SECONDS:-5}
LONG_SOURCE_INFERENCE_CONCURRENCY=${SAM_CONTINUOUS_SOURCE_SCAN_LONG_INFERENCE_CONCURRENCY:-2}
...
exec nice -n 10 "$MODEL_PYTHON" -m sam_audio_pipeline.source_pipeline scan \
  ...
  --target-scan-windows "$TARGET_SCAN_WINDOWS" \
  --max-window-hop-seconds "$MAX_WINDOW_HOP_SECONDS" \
  --long-source-inference-concurrency "$LONG_SOURCE_INFERENCE_CONCURRENCY"
